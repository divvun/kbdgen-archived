...
class SVGGenerator(Generator):
    ...
    def _make_key_group(self, primary, secondary, cls=None):
        ...
        p = SubElement(g, 'text', **{"dy": "1em",
                         "y": "32", "x": "32", "class": "key-text-primary"})
        try:
            p.text = primary
    ...
    def generate_svg(self, layout, root):
        ...
        for k in itertools.chain(ISO_KEYS, ('A03',)):
            ...
            dk_dead = dk is not None and default[k] in\
                    layout.dead_keys.get('iso-default', {})

            sk = decode_u(shift.get(k, "")) or None
            sk_dead = sk is not None and shift[k] in\
                    layout.dead_keys.get('iso-shift', {})
            ...
            ak_dead = ak is not None and alts[k] in\
                    layout.dead_keys.get('iso-alt', {})

            ask = decode_u(alts_shift.get(k, "")) or None
            ask_dead = ask is not None and alts_shift[k] in\
                    layout.dead_keys.get('iso-alt+shift', {})

            ck = decode_u(caps.get(k, "")) or None
            ck_dead = ck is not None and caps[k] in\
                    layout.dead_keys.get('iso-caps', {})
