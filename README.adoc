...
.. Install the correct pyoxidizer version.
The quickest way is using `cargo install --git https://github.com/indygreg/PyOxidizer.git --rev $pyoxidizer_rev pyoxidizer`.
+
The revision used and tested on CI right now is:
+
[source,yaml]
----
include::./azure-pipelines.yml[tag=pyoxidizer_rev]
----

. Build the project using `pyoxidizer build`.
If this succeeds, the last line of the output will tell you where you can find the final executable.

TIP: You can also build the project without the original Python kbdgen features
by running `cargo build --no-default-features` (or `run`).
This is useful for developing new features (as it is faster to compile).
